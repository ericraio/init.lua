--- *ericraio.move* Move any selection in any direction
--- *Move*
---
--- ==============================================================================
---
...
---   |Move.move_selection()| and |Move.move_line()|.
---
...
--- # Setup ~
---
--- This module needs a setup with `require('ericraio.move').setup({})` (replace
--- `{}` with your `config` table). It will create global Lua table `Move`
--- which you can use for scripting or manually (with `:lua Move.*`).
---
--- See |Move.config| for available config settings.
---
...
--- as `Move.config`. See |mini.nvim-buffer-local-config| for more details.
---
--- # Comparisons ~
---
--- - 'matze/vim-move':
---     - Doesn't support vertical movement of charwise and blockwise selections.
---       While 'ericraio.move' does.
---     - Doesn't support horizontal movement of current line in favor of
---       horizontal movement of current character. While 'ericraio.move' supports
...
---@alias __move_opts table|nil Options. Same structure as `options` in |Move.config|
...
--- Module setup
---
---@param config table|nil Module config table. See |Move.config|.
---
---@usage `require('ericraio.move').setup({})` (replace `{}` with your `config` table)
Move.setup = function(config)
  -- Export module
  _G.Move = Move
...
H.apply_config = function(config)
  ...
  H.map('x', maps.left,  [[<Cmd>lua Move.move_selection('left')<CR>]],  { desc = 'Move left' })
  H.map('x', maps.right, [[<Cmd>lua Move.move_selection('right')<CR>]], { desc = 'Move right' })
  H.map('x', maps.down,  [[<Cmd>lua Move.move_selection('down')<CR>]],  { desc = 'Move down' })
  H.map('x', maps.up,    [[<Cmd>lua Move.move_selection('up')<CR>]],    { desc = 'Move up' })

  H.map('n', maps.line_left,  [[<Cmd>lua Move.move_line('left')<CR>]],  { desc = 'Move line left' })
  H.map('n', maps.line_right, [[<Cmd>lua Move.move_line('right')<CR>]], { desc = 'Move line right' })
  H.map('n', maps.line_down,  [[<Cmd>lua Move.move_line('down')<CR>]],  { desc = 'Move line down' })
